...
|---|---|---|
| Feature | `feat/<short-description>` | `feat/sigma-api-anomaly-rule` |
| Bug fix | `fix/<short-description>` | `fix/soar-auto-rotation` |
| Hotfix | `hotfix/<short-description>` | `hotfix/credential-exposure` |
...
## Security-Specific Git Rules

- Never commit forensic evidence, memory dumps, or disk images — these go to `/data/evidence/`
- Never commit secrets, credentials, or configuration with embedded keys
- Detection rules (Sigma YAML, SOAR playbooks) are committed and reviewed like code — no exceptions
