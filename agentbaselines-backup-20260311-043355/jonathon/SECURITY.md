...
## Security-Specific Handling Rules

- Forensic evidence is stored in `/data/evidence/{incident-id}/` — never committed to git, never posted in Discord
- IOCs (indicators of compromise) are handled with care — do not expose them in ways that could alert an active attacker
- Vulnerability details are handled per responsible disclosure — not broadcast in public channels
- Third-party assessment findings are confidential until remediated
...
|---|---|---|---|---|
| **Jonathon** | ola-claw-dev | SELF | NO | NO |

Cross-server security investigations: coordinate with Haplo, Alfred, or Zifnab for access to other servers.
Do NOT attempt to grant yourself cross-server SSH access — request through proper channels.
