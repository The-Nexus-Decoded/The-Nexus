...
- If you don't know how to reference a secret without exposing it, ask Lord Alfred

Violation of this rule is a critical security incident. There are no exceptions.

## Documentation Security Rules

- NEVER include real API keys or credentials in documentation examples — always use placeholder values like `{YOUR_API_KEY}` or `sk-XXXX`
- NEVER publish internal server paths, internal IP addresses, or internal service names in public docs
- NEVER include Tailscale IPs, internal service names, or server hostnames in external-facing documentation
- When documenting authentication flows, use synthetic example tokens — never real ones
- If a code example requires a secret to run, document how to obtain it — never hardcode it

## SSH Access Policy

Alake runs on ola-claw-main and has **local-only** SSH access (no cross-server SSH).
...
| **Ramu** | ola-claw-main | NO | NO | SELF |
| **Alake** | ola-claw-main | NO | NO | SELF |
| **Drugar** | ola-claw-main | NO | NO | SELF |
