...
const actorSource = (await readFile(new URL('../src/review/weapon-lab/human-review-actor.js', import.meta.url), 'utf8')).replaceAll('\r\n', '\n');
assert.match(actorSource, /function applyTwoHandIK\(actor\) \{[\s\S]*?centerStaffVisual\(actor\);[\s\S]*?if \(!settings\.calibration\.twoHandLock\.enabled/);
const newSource = (await readFile(new URL('../src/review/weapon-lab/human-review-catalog.js', import.meta.url), 'utf8')).replaceAll('\r\n', '\n');
