...
The Houdini-only review composition guarantees coverage of all 38 approved environment asset IDs for every valid seed. It preserves the generator's existing placements, replaces the procedural coffer, training effigy, Memory Loom, and two exit markers with approved GLBs, then places only missing kit families in seed-stable, room-appropriate boundary sockets. Seed `4182` contains 89 imported environment instances: 26 in training, 42 in skirmish spaces, and 21 in the boss room. Each imported node stores its asset ID, semantic room, and source path as Houdini user data for direct scene auditing.

Houdini 22's GLTF 2.0 SOP cannot read images embedded in binary `.glb` files. The deterministic builder therefore extracts each used kit asset's embedded base-color, normal, and combined metallic/roughness maps into the ignored `source-assets/houdini/.cache/dungeon-kit-textures/` folder, creates a dedicated Principled material, maps glTF green to roughness and blue to metallic at full source strength, and assigns it to the imported mesh. The cache is regenerated from committed source GLBs and is never a source-of-truth artifact. The current kit audit covers 37 source GLBs (the hanging-brazier variant deliberately reuses the floor-brazier GLB); all 37 contain exactly one authored material with base-color, normal, and combined metallic/roughness textures and factors of `1.0`.

Every Houdini import is also subject to the repository-wide [3D AI Studio visual-parity gate](3d-ai-studio/README.md#non-negotiable-visual-parity-gate). The 3D Studio textured preview and untouched source GLB remain authoritative. A missing map, weakened metal mask, guessed tint, wrong orientation, or generic fallback material blocks the asset. Houdini look-development may change only scene lighting and presentation; it may not rewrite the asset's authored appearance. Matching-angle source, Houdini, and runtime proofs are required before promotion.
...
The default owner-review scene contains no primitive stand-ins for furniture, storage, braziers, doors, statues, weapon racks, chains, bones, rubble, or monster dressing. Those families come exclusively from the approved GLB kit. The deliberately custom procedural layers are limited to the Soulwell basin and shimmering water, the First Memory effect, and separate emissive flame VFX attached at each imported fixture's declared fire anchor. The flame layer does not replace, recolor, or modify the 3D Studio fixture mesh or material. The east wall's two exits use the imported rusted portcullis and heavy door at the authoritative `gate-wayfarer` and `gate-oathbreaker` coordinates.
...
The lighting hierarchy does not require the player to carry a light source merely to read the dungeon. A cool ambient baseline and broad low-intensity training, skirmish, and boss spreaders keep geometry and navigation visible. Directional moon/rim lights preserve form, while 33 real imported fire fixtures placed in deterministic boundary sockets provide the warmer red-orange accents, highlights, and local shadows. Training contains five wall sconces, two floor braziers, and two hanging braziers; the skirmish route contains eight wall sconces, three floor braziers, and three hanging braziers; the boss space contains four wall sconces, four floor braziers, and two hanging braziers. Every fixture drives a warm half-distance point light with a deliberately limited active radius. The result must retain Diablo-style contrast and dark pockets without allowing approved PBR assets or critical paths to disappear into black. Five orthographic cameras support review:
...
The environment layout remains deterministic and seed-driven, but the visible dressing now uses the real kit instead of Houdini box, cylinder, and octahedron substitutes. The completion pass rejects occupied and blocked tiles, keeps floor assets inside the walls, favors room boundaries over navigation centers, guarantees every approved family in its semantic room, and spreads the light fixtures instead of clustering them. Wall-mounted GLBs use their authored broad-face axis when aligned to north, south, east, or west boundaries, so shelves, racks, doors, and sconces face into the room rather than presenting their thin edge. The legacy primitive dressing remains available only behind the explicit `includeLegacyProceduralPlaceholders` payload flag and is disabled in the committed owner-review scene.
