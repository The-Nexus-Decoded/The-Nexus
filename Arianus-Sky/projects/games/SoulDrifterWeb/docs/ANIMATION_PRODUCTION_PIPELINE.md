...
14. **Contact actions require context-bearing previews.** Any action that touches, carries, manipulates, climbs, strikes, opens, closes, locks, mines, chops, lifts, places, or otherwise depends on an object or surface must be previewed with a correctly scaled proxy prop or environment contact. A body moving against empty space cannot prove hand placement, reach, leverage, contact, clearance, or object motion. The proxy is QA-only and must never be baked into the animation or character; the runtime object remains a separate entity attached through the action's sockets and contact markers.
15. **A missing motion must be newly authored.** Once the master demand matrix marks a humanoid action `MISSING`, do not close that gap by reversing, mirroring, relabeling, trimming, splicing, crossfading, or combining parts or poses from existing clips. Do not layer one clip's upper body over another clip's lower body and call the result a new action. Author a new whole-body motion from the accepted rest rig using explicit keyframes, IK, constraints, contacts, and the recorded real-person reference. Source-derived transformations may create variants only after the underlying semantic motion is already covered and owner-approved; they cannot fill a missing semantic row. The issue #487 source-derived gap packs are rejected process evidence and are forbidden from runtime integration or further owner review.
16. **The zero-action T-pose is rest/rig authority only.** The accepted 65-bone source supplies skeleton identity, hierarchy, rest transforms, scale, and the no-action import boundary. It is not a natural gameplay stance or motion source, and it must never appear at a playable clip boundary or between authored beats.
17. **Organic motion requires dense authoring, not sparse interpolation.** A script may automate Blender setup, helpers, constraints, sampling, evidence, bake, export, and validation, but a few milestone poses joined by interpolation cannot pass as finished human mechanics. Original gaps require reference-driven dense/full-frame body and IK/contact work, center-of-mass and motion-path review, curve and velocity inspection, and complete normal-speed side plus three-quarter visual review. Numeric checks cannot promote visibly bad motion.
...
### 1C. Author original gaps through the dense Blender pass

The current issue #487 custom-gap route is `BLENDER_DENSE_FULL_FRAME`. Cascadeur and video-mocap drafting are deferred and recorded as `DEFERRED_NOT_USED`; lack of those tools is not a blocker for the current pass.

1. Use the accepted zero-action T-pose only to establish the authoritative rig/rest state. Create the declared natural gameplay start and recovery poses before animation playback begins.
2. Build a complete stepped/Constant blocking pass from the recorded real-person mechanics: anticipation, loading, contact or release, follow-through, settle, and recovery. Blocking locates the beats; it is not a finished sparse interpolation.
3. Author the organic body mechanics densely. Evaluate pelvis, spine, shoulders, hands, legs, feet, IK, pole targets, prop/weapon targets, and contact constraints on every frame where they matter; bake the evaluated result at one-frame sampling.
4. Review pelvis/center-of-mass travel against the changing support base and confirm the visible transfer of weight through feet, knees, hips, torso, shoulders, and hands.
5. Calculate Blender motion paths for pelvis, hands, feet, and relevant prop/weapon controls. Programmatically sample transforms, curves, linear velocity, and angular velocity across every consecutive frame and reject unexplained spikes, flips, stalls, grip drift, or sliding.
6. Polish timing, spacing, ease, overshoot, and rotational continuity in the F-Curves. Blender GUI/Graph Editor review is preferred, following the official [F-Curve Properties](https://docs.blender.org/manual/en/4.5/editors/graph_editor/fcurves/properties.html) guidance, but a recorded programmatic F-Curve workflow is acceptable and GUI availability is not a gate. Use Blender's official [Motion Paths](https://docs.blender.org/manual/en/latest/animation/motion_paths.html) and [Inverse Kinematics Constraint](https://docs.blender.org/manual/en/latest/animation/constraints/tracking/ik_solver.html) behavior as the implementation baseline.
7. Bake constraints, export the exact candidate, fresh-reimport it, and verify the 65-bone skeleton, contacts, grounding, duration, boundaries, and artifact hash.
8. Render the full body and required prop/weapon path at normal gameplay speed from separate side and three-quarter cameras. The author watches every frame of both decoded videos and clears all visible blockers before quarantine handoff. The independent reviewer then watches the exact hashed playback; neither technical metrics nor author self-review can substitute for that verdict.

Record this evidence in `technicalReview.evidence.organicAuthoringWorkflow`. The executable gate rejects original Tier 3 receipts that declare the rest rig as a motion source, use sparse scripted pose synthesis alone, omit full-frame constraint evaluation, leave undocumented or catastrophic velocity spikes, skip either full-motion camera view, or rely on technical metrics for promotion.

For pilot throughput, `PROVISIONAL_PILOT` may record minor contact offset, easing, or curve-polish cleanup in explicit `polishNotes` while all semantic, anatomical, grounding, prop, source-rig, provenance, hash, and lifecycle gates remain passing. When the first full-motion review finds only minor deviations, perform one corrective build and repeat the two-angle self-review; if only minor polish remains, write the notes, mark provisional, and move to the next action rather than entering another polish loop. Minor deviation counts and notes must agree, independent review must still watch the complete normal-speed playback, and owner approval is still required before BREACH-V2 queueing. Wrong action, broken/snapping limbs, T/bind-pose boundaries, floor/body/prop penetration, lost required grip/contact, root/grounding failure, catastrophic discontinuity, missing evidence, or hash/promotion mismatch is never provisional. `SHIPPING_VERIFIED` requires a full `PASS` with no remaining polish notes.
