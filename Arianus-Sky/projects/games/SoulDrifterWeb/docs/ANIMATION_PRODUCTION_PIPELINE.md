...
13. **Every humanoid enters animation through the dual-pose and Smart Mesh gate.** Tripo bodies use the live, explicitly receipted Smart Mesh model with accepted Quad Face topology before PBR texturing or rigging. Record the exact provider version and re-audit native-quad capability when the label changes; never substitute HD Model. The canonical rig preserves a strict T-pose calibration artifact and derives a relaxed A-pose validation artifact from the same mesh and skeleton. Neither the provider label nor one attractive pose waives joint-loop, skin-weight, equipment-fit, or continuous-deformation proof.
