...
- **Anwel river-town**: five hollow, enterable timber-framed houses (stone footing, plank floor, plaster panel walls with **2.2 m open doorways** facing the plaza and a window on the back wall, timber corner posts and door surrounds, thatch/slate prism roofs) around the village well, plus dock and road fences. The river course and the north-south road both pass through the atlas anchor point, so the village sits ~7 m east on the plaza — beside the road, off the wet bank.
...
- **Poly Haven dressing** (`HEARTVALE_POLYHAVEN` geo node): CC0 props and vegetation from `source-assets/polyhaven/` (see `third-party-assets.json`) — barrel and crate clusters on the dock and at the store barn, buckets at the plaza well, a lantern hung beside every house door at 2 m, two grass species scattered as verge/meadow clumps, wild rooibos bushes, and four full-geometry hero trees (terrace, riverbank, Anwel entrance, south road) normalized to 6.5–8 m. All chains are **live file references** (`$HIP/../polyhaven/<id>/<id>.gltf` → unpack → height-normalize → place/scatter), so the committed hip stays lean. Materials are generated per asset from the glTF's own material→texture map (`/mat/PH_*`, `basecolor_texture` wired, point-color tint off). Note: `tree_small_02` and `island_tree_02` ship with `KHR_texture_transform` marked **required**, which Houdini's glTF importer rejects — the local `.gltf` copies have that extension stripped (geometry and textures unchanged; recorded in `third-party-assets.json`).
