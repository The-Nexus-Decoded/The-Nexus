...
## Scoped QA integration checkpoint

Source checkpoint: `55e4311f070e412ebc14f1d2395e254bfc1150f1` on `codex/435-tripo-modular-character-foundation-v2` (pushed). The integration branch is `codex/435-lab-qa-release`, based on QA `2157dcd8ba485e9c558ca1c75e53051f2dd4277d`.

Only the new package, nine existing mapped GLBs, and three linked provenance/calibration documents are included. QA's existing game code, pruner, workflows, dependencies, protected collections and asset-license records are unchanged. The manifest adds 26 protected review paths; the license registry adds 24 records.

The QA-based build and release verifier passed: dist 317,776,755 bytes and dist-pages 317,766,322 bytes before final metadata commit. Final commit identifiers and merged-pipeline results are recorded in the PR and issue completion comment.

Windows checkout note: system Git `core.autocrlf=true` converted the ten protected dungeon JSON files to CRLF in the fresh worktree. Each Git/index blob already matched its protected SHA; LF normalization restored identical bytes without content or tracked changes. No guard, expected hash, dungeon file, or Git configuration was changed in this release. Rebuild after correcting local checkout line endings; do not weaken protection checks.

Existing diagnostic limitation (deferred, no fix): `scripts/export-breach-v2-topology.mjs:72` captures the full `git diff --binary HEAD` through `execFileSync` with its default output buffer. With the large uncommitted asset package this raises `ENOBUFS`; before staging it rejects untracked inputs instead. Run the full QA suite again from the clean committed package, as CI does. The other 295 tests passed in the pre-commit run. A later tooling ticket can stream/hash the diff or set an appropriate bound; this release does not alter the exporter or suppress its test.
