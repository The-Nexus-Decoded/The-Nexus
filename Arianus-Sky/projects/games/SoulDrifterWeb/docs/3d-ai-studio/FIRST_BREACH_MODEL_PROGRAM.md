...
The rejected paid Warden remains provenance only. Its replacement source set combines corrected front v2, exact left profile v3, true rear v2, and exact right profile v3. The front proves the single sensor mask, front-only furnace cavity, left soul-tax palm, and right integrated blade; the rear proves a solid head shell, segmented mechanical spine, and two heat vents with no second face or core; the side elevations collapse the front furnace to edge thickness and preserve the asymmetric near/far arms. All four 1122x1402 sources passed the contradiction gate. After the owner removed the temporary 800-credit reserve, Meshy 7 Multi-Image task `ca97a8d6-2fc0-4c9a-8ed4-8cf7eb6d3764` converted those exact four sources for 45 credits. The preserved 68,660,976-byte GLB passes the one-head/one-front, solid-rear, asymmetric-arm, core-cavity, intact-limb, and no-floating-duplicate gates. At 1,985,294 triangles with no skin or animations, it is a provisional production source for later retopology, rigid part hierarchy, `VFX_CoreFlame`, and gameplay proof rather than a runtime drop-in.
...
- Preserve the complete full-spec Oathbound and Ravager v3 four-view sets and submit neither Meshy conversion until the owner reviews the sources and the account has enough headroom to keep the balance at or above the 800-credit hard floor.
