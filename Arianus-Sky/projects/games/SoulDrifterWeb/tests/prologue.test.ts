...
describe("Chronicle of Returning", () => {
  it("separates historical Cycle pages from the Sundered Spheres continuation and ends with the active soul", () => {
    ...
    expect(pages.find((page) => page.id === "souldrift")?.era).toBe("Sundered Spheres continuation");
    ...
    expect(pages.find((page) => page.id === "souldrift")?.narration.join(" ")).toContain("Sphere Drifters are dead mensch returned");
