...
function inPlaceGroundedClip(
  ...
  // The immutable Mixamo library authors locomotion on Hips, while small test
  // and Blender-authored packs may place it on the armature object itself.
...
export function createBreachV2HumanFoundationActor(
  ...
  model.scale.setScalar(options.heightMeters / sourceHeight);
  ...
  return {
    ...
    update: (deltaSeconds) => {
      ...
      // The accepted GLB is a skinned clone. Its CPU skinned bounds become
      // authoritative only after the actor has joined the live scene and its
      // first animation frame has evaluated. Calibrating earlier measures the
      // imported bind pose and buries the animated feet by roughly 0.33 m.
      const grounding = calibrateAnimatedPoseOnFloor(root, model, groundingPivot, 0);
      // Three's immediate CPU bounds probe counts an attached SkinnedMesh
      // parent translation in both the bone matrices and matrixWorld. The next
      // renderer update applies that parent translation once. Convert the
      // measured correction back to the render-space parent displacement so
      // the visible feet, rather than the transient CPU probe, meet the floor.
...
export async function createBreachV2HumanFoundationActorFactory(
  ...
  const catalogLoader = new PilotAnimationCatalogLoader(catalog, loader, 1);
  const [idle, walk] = await Promise.all([
    catalogLoader.loadClip(FOUNDATION_IDLE_CLIP),
    catalogLoader.loadClip(FOUNDATION_WALK_CLIP),
  ]);
