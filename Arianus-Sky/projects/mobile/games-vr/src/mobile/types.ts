...
// ==================== Immersion Depth (User State → Architecture) ====================
// Per Samah: "ImmersionDepth is a description of the user's relationship to the space"
// Not a technical flag — UX behavior defines the contract

export type ImmersionDepth = 'stepping_out' | 'fully_in' | 'cooling_down';

export interface ImmersionState {
  depth: ImmersionDepth;
  attentionLease: boolean;    // Visual handoff fade active
  uiExpanded: boolean;         // UI expands to z-space
  notificationsSuppressed: boolean;
  timestamp: number;
}

export const IMMERSION_BEHAVIOR: Record<ImmersionDepth, {
  uxBehavior: string;
  attentionLease: boolean;
  uiExpanded: boolean;
  notificationsSuppressed: boolean;
}> = {
  stepping_out: {
    uxBehavior: 'Visual handoff fade',
    attentionLease: true,
    uiExpanded: false,
    notificationsSuppressed: false,
  },
  fully_in: {
    uxBehavior: 'UI expands to z-space, notifications suppressed',
    attentionLease: false,
    uiExpanded: true,
    notificationsSuppressed: true,
  },
  cooling_down: {
    uxBehavior: 'UI contracts gracefully, awareness returns',
    attentionLease: false,
    uiExpanded: false,
    notificationsSuppressed: false,
  },
...
// ==================== Gesture → Immersion Mapping ====================

export interface GestureImmersionMap {
  gesture: GestureType;
  targetDepth: ImmersionDepth;
  transitionDuration: number;  // ms
}

export const GESTURE_IMMERSION_MAP: GestureImmersionMap[] = [
  { gesture: 'double_tap', targetDepth: 'fully_in', transitionDuration: 300 },
  { gesture: 'long_press', targetDepth: 'stepping_out', transitionDuration: 500 },
  { gesture: 'rotate', targetDepth: 'cooling_down', transitionDuration: 200 },
];
