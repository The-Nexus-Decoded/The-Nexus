...
- `shared/anewluv/` — ANewLuv shared product and planning docs
...
### ANewLuv
- Product channel is now defining beta relationship logic, not just brand posting
- Consensus beta model saved to `shared/anewluv/ANewLuv-beta-match-like-chat-spec-v1.md`
- Final agreed model: Match = system-surfaced fit, Like = one-way interest, Intro = one optional opener, Connection = mutual like or intro reply, Chat unlocks only after connection
- Score/tier explains quality and is kept separate from social/chat permission state
- Owner context: Sinistrad (coordination), Lord Xar (content/product direction)
