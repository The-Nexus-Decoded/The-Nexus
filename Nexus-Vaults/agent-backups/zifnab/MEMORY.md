...
- **Account:** @AnewluvDGOD (credentials path: `/data/openclaw/shared/secrets/anewluv-x-account.txt`; do not print values)
...
- [x] ~~Drugar/Devon Codex EXP fix (2026-05-06): Grundel applied non-secret auth-profile copy/lastGood fix for drugar on dev and devon on trade; backups created and gateways restarted. #jarvis heartbeat at 14:11 CDT showed both `UP` on `openai-codex/gpt-5.5` with no `EXP` tag. Dormant expired Codex profiles on MiniMax agents were left untouched.~~
- [x] ~~Rega heartbeat false mustache (2026-05-07): heartbeat-grid.py was selecting Rega's idle direct `agent:main:main` session (170k/200k, 85%) instead of active #jarvis session (41-42k/200k, 21%). Alfred changed `/data/openclaw/scripts/heartbeat-grid.py` so CTX prefers `agent:main:discord:channel:1475082997027049584` and added `SRC` (`jarvis/channel/direct`); tag semantics changed so fallback Codex OAuth auth behind healthy non-Codex primary is `FBAUTH` warning, not `AUTH` degradation. `python3 -m py_compile` passed; backups exist at `/data/openclaw/scripts/heartbeat-grid.py.bak-session-source-20260507-080501` and `/data/openclaw/scripts/heartbeat-grid.py.bak-fbauth-20260507-080750`. 10:00 CDT heartbeat verified clean render: Rega `43k/200k 22%` with `SRC=jarvis`, no degraded alert.~~
- [ ] Rega Codex OAuth fallback auth (2026-05-07): stale fallback profile returns `refresh_token_reused`; primary MiniMax is healthy. This is an operator-only interactive TTY re-auth: `openclaw --profile rega models auth login --provider openai-codex`. Do not restart Rega and do not attempt headless auth.
...
## /home/openclaw/.openclaw-zifnab/workspace/HEARTBEAT.md
