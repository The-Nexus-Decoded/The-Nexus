#!/usr/bin/env python3
"""
Meteora DLMM Signal Scanner Runner

Starts the scanner in a loop with dedicated health endpoint.
Configuration: environment variables or --config file (YAML/JSON).
"""
...
SERVICE_SRC = os.path.join(os.path.dirname(__file__), "hughs-forge", "services", "trade-orchestrator", "src")
...
try:
    ...
    print(f"Failed to import scanner modules: {e}", file=sys.stderr)
    sys.exit(1)
...
def main():
    ...
    logging.info(f"Scanner starting. Health endpoint: http://0.0.0.0:{config.health_port}/health")
    ...
    if args.once:
        ...
        time.sleep(2)  # Allow health server to respond if needed
        return 0
