...
Both positions are actively managed, not fire-and-forget. The service subscribes to the pool's active-bin account over Solana WebSocket. Every active-bin update schedules a deterministic range check. If the subscription drops, a 15-second poll is the fallback while the service reconnects; a full position and accounting snapshot runs every 60 seconds. The tight Spot sleeve and wide BidAsk ladder alert within two bins of an edge. An enrolled retarget executes only after the position is actually outside its range, uses a fresh active bin and simulation, and never reduces the wide sleeve merely to fund the tight sleeve.
...
SPARKY currently has auto-compounding disabled. Before campaign break-even, accrued fees are held and tracked separately from LP principal. The break-even gate is current executable value of every campaign position plus cumulative net fees earned, less all execution costs, compared with the original campaign entry basis. After that gate opens, each economical realized fee batch is converted to SOL and swept to the configured profit wallet. Fees already swept remain in the cumulative campaign-return ledger, so sweeping them cannot move the configured close target farther away.
...
Profit sweeping is designed to stop successful campaigns from automatically compounding their winnings into the next trade. During an active non-compounding campaign, fee-only sweeps begin after the combined position-plus-fee ledger reaches break-even. Position liquidity never funds those fee sweeps. At the configured profit target, the executor closes every campaign position, claims remaining fees, converts campaign tokens to SOL, and reconciles every close and swap.

Only proven campaign profit is eligible for transfer. Returned principal remains in the trading wallet and is never part of the sweep. Post-close settlement looks only at SOL attributable to the exit transactions; unrelated pre-existing wallet SOL is excluded. The original entry basis stays in the trading wallet and only exit proceeds above that basis, after transfer cost, go to the distinct profit wallet. Each transfer is simulated, uses a settlement fingerprint and one idempotency key, and is recorded only after finalized on-chain reconciliation.
...
The Sparky campaign is the first live-capable two-position proof. Its POC target is campaign-specific 40%; the reusable engine remains configurable from 10% through 80%. Before live capital movement, the report must show the fresh active bin, original 2.269634845 SOL campaign basis, 3.177488783 SOL close target, prior realized proceeds and fees, current inventory, protected reserve, wide and tight allocations, bin-by-bin proceeds, all costs, hold-relative comparison, and every rejected guard.

The local signer uses a resumable public journal because closing the root and opening an extended wide position plus a tight position requires multiple transactions. Every stage is simulated immediately before send, records its signature before broadcast, and reconciles confirmed state on resume. A live opening requires the exact SPARKY campaign enrollment, the explicit mainnet confirmation phrase, all root-close simulations, adequate post-close rent evidence, and fresh staged child simulations. Success means exactly two descendant positions reconcile on chain, fees remain separate when auto-compounding is off, and the 15-second controller continues range, fee, target-close, and vault-settlement management.
