param(
    [string]$ToolRoot = "H:\CodexData\souldrifter-toolchain\tripo-v3",
    [string]$ReceiptRoot = "H:\CodexData\souldrifter-toolchain\receipts",
...
if (-not $env:TRIPO_API_KEY) {
    Write-Blocked "TRIPO_API_KEY is not present in the process environment. Store it in an approved local secret location; do not paste it into chat or Git." 14
}
...
try {
    ...
    $receipt = [ordered]@{
        schemaVersion = 1
        toolchainSchemaVersion = "2026-08-24-toolchain-v1"
        receiptId = $receiptId
        generatedAt = (Get-Date).ToUniversalTime().ToString("o")
        provider = "Tripo3D"
