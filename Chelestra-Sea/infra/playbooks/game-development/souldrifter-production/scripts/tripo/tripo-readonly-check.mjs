...
try {
  ...
  try {
    ...
    // The authenticated read is still authoritative; package version can be
    // populated by the bootstrap wrapper if package metadata is unavailable.
  }

  const availableBalance = pickNumber(balanceResponse, [
    'balance',
    'available_balance',
    'available',
    'credits',
  ]);
  const frozenBalance = pickNumber(balanceResponse, [
    'frozen',
    'frozen_balance',
    'reserved',
  ]);
