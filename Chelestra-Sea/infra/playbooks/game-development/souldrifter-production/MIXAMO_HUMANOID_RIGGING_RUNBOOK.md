...
- Keep the exact approved Smart Mesh head as neutral `Basis`; use a separate temporary Blender Rigify authoring rig with `face.skin_eye`, `face.skin_jaw`, and `face.basic_tongue`; bake the complete 52-name ARKit/MediaPipe-compatible expression contract and the 15-name Meta-style speech-viseme contract; then strip every generated facial-rig object, bone, driver, constraint, modifier, and action before fresh-import proof of the unchanged 65-bone runtime skeleton.
